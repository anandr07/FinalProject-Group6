...
def train_model(report_csv_path: str, labels_csv_path: str, image_dir: str, num_epochs: int = 10,
    ...
    try:
        ...
        # Optimizers
        model_optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5)  # Lower learning rate for BiomedCLIP
        ...
        # Loss function for BiomedCLIP
        clip_loss = nn.MSELoss()
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                try:
                    ...
                    # Get image features from BiomedCLIP (now with gradients)
                    image_features = model.encode_image(images)
                    text_features = model.encode_text(tokenizer(texts).to(device))  # Encode the findings text
                    ...
                    contrastive_labels = torch.arange(len(images), device=device,
                                                      dtype=torch.long)  # Changed variable name and specified dtype
                    ...
                    # Combined loss
                    total_loss = gen_loss * 0.5 + clip_loss_value  # Adjust weight as needed

                    # Track losses
                    if not torch.isnan(total_loss):
                        train_losses.append(total_loss.item())
                        clip_losses.append(clip_loss_value.item())
                        gen_losses.append(gen_loss.item())  # Add this line
                    ...
                    progress_bar.set_postfix_str(
                        f'T={total_loss.item():.4f}, G={gen_loss.item():.4f}, C={clip_loss_value.item():.4f}'
                    )
            ...
            avg_gen_loss = sum(gen_losses) / len(gen_losses)  # Add this line

            logger.info(f"\nEpoch {epoch + 1} Stats:")
            logger.info(f"Total Training Loss: {avg_train_loss:.4f}")
            logger.info(f"Generator Loss: {avg_gen_loss:.4f}")  # Add this line
