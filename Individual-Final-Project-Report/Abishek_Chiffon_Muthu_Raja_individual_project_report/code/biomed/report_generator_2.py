...
class MedicalReportGenerator(nn.Module):
    ...
    def generate_reports_batch(self, input_embeddings, labels=None, max_new_tokens=512, num_beams=4):
        """
        ...
        try:
            ...
            if len(projected_embeddings.shape) == 2:
                projected_embeddings = projected_embeddings.unsqueeze(0)
            ...
            outputs = self.model.generate(
                ...
                eos_token_id=self.tokenizer.eos_token_id,
                use_cache=False  # Disable cache to work with gradient checkpointing
            )

            # Decode all outputs at once
            reports = self.tokenizer.batch_decode(outputs, skip_special_tokens=True)

            # Filter out empty or None reports
            reports = [report if report else "" for report in reports]

            return reports

        except Exception as e:
            logging.error(f"Error in generate_reports_batch: {str(e)}")
            # Return empty strings in case of error
            return [""] * input_embeddings.size(0)
